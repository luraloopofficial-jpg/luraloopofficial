...
import { 
  Stethoscope, CalendarCheck, Clock, ShieldCheck, MessageCircle, Server, 
...
export default function HealthcareContent() {
  ...
  const features = [
    { title: "Appointment Booking", icon: <CalendarCheck className="w-6 h-6 text-brand-orange" /> },
    { title: "Doctor Availability", icon: <Stethoscope className="w-6 h-6 text-brand-orange" /> },
    { title: "HMS Integration", icon: <Server className="w-6 h-6 text-brand-orange" /> },
    { title: "Secure & Private", icon: <ShieldCheck className="w-6 h-6 text-brand-orange" /> },
    { title: "WhatsApp Based", icon: <MessageCircle className="w-6 h-6 text-brand-orange" /> },
    { title: "24/7 Availability", icon: <Clock className="w-6 h-6 text-brand-orange" /> },
  ];
  ...
  const integrations = [
    "HMS Systems", "WhatsApp Business API", "Google Calendar", "Internal Workflows", "Custom APIs"
  ];
    ...
    <div className="space-y-10 text-brand-white">
      ...
      <div className="relative w-full overflow-hidden border-b border-white/10 pb-8 mb-6">
        <div className="absolute right-0 top-0 w-full md:w-1/2 h-full z-0 opacity-20">
          <Image src="/modal-healthcare.png" alt="Healthcare Background" fill className="object-cover object-right" />
        </div>
        ...
        <div className="relative z-20 flex flex-col items-start text-left pt-2">
          <h3 className="text-3xl font-bold mb-3 text-brand-white">Your Hospital's Intelligent Front Desk.</h3>
          <p className="text-brand-gray text-lg max-w-xl leading-relaxed">
            24/7 appointment booking, patient enquiries, doctor schedules, and hospital information — instantly through WhatsApp.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-8 border-t border-white/10">
        <div className={`p-6 bg-brand-orange/5 border border-brand-orange/20 rounded-2xl ${hoverClasses}`}>
          ...
          <p className="text-sm text-brand-gray/90 leading-relaxed">
            Reduce front-desk workload, increase appointment booking conversions, and provide instant 24/7 support without adding additional operational headcount.
          </p>
        </div>
